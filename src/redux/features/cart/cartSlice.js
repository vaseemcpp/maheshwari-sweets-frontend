...
const cartSlice = createSlice({
  ...
  reducers: {
    ADD_TO_CART(state, action) {
      ...
      if (productIndex >= 0) {
        ...
        if (cartQuantity === action.payload.quantity) {
          ...
          state.cartItems[productIndex].cartQuantity += 0.5;
          toast.info(`${action.payload.name} increased by 0.5 Kg`, {
            position: "top-left",
          });
        }
      } else {
        ...
        toast.success(`1 Kg ${action.payload.name} added to cart`, {
          position: "top-left",
        });
      }
      // save cart to LS
    ...
    DECREASE_CART(state, action) {
      ...
      if (state.cartItems[productIndex].cartQuantity > 0.5) {
        state.cartItems[productIndex].cartQuantity -= 0.5;
        toast.info(`${action.payload.name} decreased by 0.5 Kg`, {
          position: "top-left",
        });
